...
#+BEGIN_SRC emacs-lisp
  ...
  (general-define-key
    ...
    "tn" 'org-table-create
    "ti" '(lambda () (interactive) (let ((current-prefix-arg '(0))) (call-interactively 'org-time-stamp-inactive)))
    )
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package haskell-mode
	...
	(append
	  (append (list "nix-shell" "-I" "ssh-config-file=/home/sam/.ssh/nixbuild.config" "--command" )
		  (list (mapconcat 'identity (append args (list "--ghc-option=-Wwarn")) " "))
	    )
	    (list (nix-current-sandbox))	
	  )
	)
    ...
    (setq haskell-hoogle-server-command (lambda (port) (apply default-nix-wrapper (list (list "hoogle" "server" "--local" "-p" (number-to-string port))))))
